...
def main(data_dir='./data'):
  ...
  for task_name in ["chunk"]:
    ...
    train_sentences = word_level_data.TaggedDataLoader(
        config, task_name,False,2).get_labeled_sentences("train")
    random.shuffle(train_sentences)
    l=len(train_sentences)
    split=int((l-1500)/3)
    write_sentences(task_data_dir + 'train_subset_0.txt', train_sentences[1500:1500+split])
    write_sentences(task_data_dir + 'train_subset_1.txt', train_sentences[split+1500:1500+2*split])
    write_sentences(task_data_dir + 'train_subset_2.txt', train_sentences[split+1500+2*split:])
  ...
  for task_name in ["chunk"]:
    for i, label_encoding in enumerate(["BIOES"]):
      ...
      loader = word_level_data.TaggedDataLoader(config, task_name, token_level,2)
      if token_level:
        if i != 0:
          continue
