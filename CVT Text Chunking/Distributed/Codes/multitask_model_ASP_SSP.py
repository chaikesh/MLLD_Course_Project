...
class Model(object):
  def __init__(self, config, pretrained_embeddings, tasks):
    ...
    with tf.variable_scope('unsup'):
      ...
      self._unlabeled_train_op = self._get_train_op(self._unlabeled_loss,0)
    self._labeled_train_ops = {}
    for task in self._tasks:
      
      with tf.variable_scope('sup'):
        task_loss = self._trainer.modules[task.name].supervised_loss
        self._labeled_train_ops[task.name] = self._get_train_op(task_loss,1)
  ...
  def _get_optimizer(self):
    ...
    ##################################################################################ADDED IN CASE OF SSP##########################################################
    
    return global_step, optimizer

  def _get_train_op(self, loss,flag):
    replicated_opt=tf.contrib.opt.DropStaleGradientOptimizer(opt=self._optimizer,staleness=self._config.stale_parameter)
    grads, vs = zip(*replicated_opt.compute_gradients(loss))
    grads, _ = tf.clip_by_global_norm(grads, self._config.grad_clip)
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
      return replicated_opt.apply_gradients(
