...
class BiGRU_CTC():
    ...
    def __init__(self,FLAGS,num_hidden = 200,num_layers = 2,num_features=40,initial_learning_rate = .001,
        ...
        self.parameter_servers=["10.1.1.254:2324"]; ##  node 0 is 254  node 1 is 253 node 2 is 252 and node 3 is 251 node 4 is 249 
        self.workers=["10.1.1.253:2325","10.1.1.249:2326"];   
...
# config.gpu_options.allow_growth=True;
    ...
    ###############################
                ...
                with tf.variable_scope("sup"):
                    ...
                    self.optimizer_supervised = tf.train.MomentumOptimizer(self.initial_learning_rate,0.9);

                    self.replicated_opt_supervised = tf.contrib.opt.DropStaleGradientOptimizer(opt=self.optimizer_supervised,staleness=self.stale_parameter_sup);
            
                    self.final_optimizer_supervised = self.replicated_opt_supervised.minimize(self.final_cost_supervised, global_step=self.global_step);
                ...
                with tf.variable_scope("unsup"):
                    ...
                    self.optimizer_unsupervised = tf.train.MomentumOptimizer(self.initial_learning_rate/50,0.9);

                    self.replicated_opt_unsupervised = tf.contrib.opt.DropStaleGradientOptimizer(opt=self.optimizer_unsupervised,staleness=self.stale_parameter_unsup);
            
                    self.final_optimizer_unsupervised = self.replicated_opt_unsupervised.minimize(self.final_cost_unsupervised, global_step=self.global_step);
            ...
            with self.sv_obj.prepare_or_wait_for_session(self.server.target) as session:
                ...
                for curr_epoch in range(num_epochs):
                    ...
                    for batch in range(num_batches_per_epoch):
                        ...
                        batch_cost, _ = session.run([self.final_cost_supervised, self.final_optimizer_supervised], feed)
                        
                        sup_train_cost += (batch_cost*batch_size)/num_examples;
                        # print(batch_cost);
                        ...
                        if curr_epoch>0:
                            ...
                            batch_cost, _ = session.run([self.final_cost_unsupervised, self.final_optimizer_unsupervised], feed)
                            unsup_train_cost+= (batch_cost*batch_size_unsup)/len(unlabeled_tr_inputs);
                    ...
                    log_file = open('worker_ssp_2_log'+str(self.FLAGS.task_index)+'.txt',"a");
                ...
                ## for model saving
                    ...
                    if(curr_epoch==250):
                        self.saver.save(session, './model_cvt_same_batch_ssp_sup'+str(self.FLAGS.task_index)+'/model.ckpt');
                        print("Model saved in path: %s" % './model_cvt_same_batch_ssp_sup'+str(self.FLAGS.task_index)+'/model.ckpt');
