...
class BiGRU_CTC():
    ...
    def __init__(self,FLAGS,num_hidden = 200,num_layers = 2,num_features=40,initial_learning_rate = .001,
                reg_constant = 0.05,allow_growth_gpu=True,top_paths=1,beam_width=1,stale=10):
        


        self.parameter_servers=["10.1.1.254:2228"]; ##  node 0 is 254  node 1 is 253 node 2 is 252 and node 3 is 251 node 4 is 249 
        self.workers=["10.1.1.253:2225","10.1.1.251:2226","10.1.1.249:2223"];##   
        self.cluster = tf.train.ClusterSpec({"ps":self.parameter_servers,"worker":self.workers});
        self.FLAGS=FLAGS;
...
# config.gpu_options.allow_growth=True;
        ...
        self.server=tf.train.Server(self.cluster,job_name=self.FLAGS.job_name,task_index=self.FLAGS.task_index,config=self.config)
        ...
        if self.FLAGS.job_name=='ps':
            ...
            with tf.device(tf.train.replica_device_setter(
                ...
                self.stale_parameter_sup=(stale);
                self.stale_parameter_unsup=(stale);
                ...
                self.sup_flag=tf.placeholder(tf.float32);
...
##########################################################################
  ...
  # 
                # if(self.sup_flag==1):
                with tf.variable_scope("sup"):

                    self.loss_sup = tf.nn.ctc_loss(self.targets, self.logits, self.seq_len) ##### SEE the sparse targets
                
                    self.cost_sup = tf.reduce_mean(self.loss_sup);

                    self.final_cost_sup=self.cost_sup+self.reg_constant*sum(self.reg_losses);
                
                    self.optimizer_sup = tf.train.MomentumOptimizer(self.initial_learning_rate,0.9);

                    self.replicated_opt_sup=tf.train.SyncReplicasOptimizer(opt=self.optimizer_sup,total_num_replicas=len(self.workers)\
                                                      ,replicas_to_aggregate=len(self.workers),use_locking=True);
        
                    self.final_optimizer_sup = self.replicated_opt_sup.minimize(self.final_cost_sup, global_step=self.global_step);


                    self.init_token_sup = self.replicated_opt_sup.get_init_tokens_op();##Returns the op to fill the sync_token_queue with the tokens.
        
                    self.queue_runner_sup = self.replicated_opt_sup.get_chief_queue_runner(); #Returns the QueueRunner for the chief to execute.
                ...
                with tf.variable_scope("unsup"):
                    ...
                    self.loss_unsup= tf.reduce_mean(tf.nn.ctc_loss(self.gold_truth, self.logits_fw, self.seq_len)+tf.nn.ctc_loss(self.gold_truth, self.logits_bw, self.seq_len));

                    self.reg_losses_unsup= tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)###########################keys
    
                    self.final_cost_unsup=self.loss_unsup+self.reg_constant*sum(self.reg_losses_unsup);

                    self.optimizer_unsup = tf.train.MomentumOptimizer(self.initial_learning_rate/50,0.9);

                    # self.replicated_opt_unsup=tf.train.SyncReplicasOptimizer(opt=self.optimizer_unsup,total_num_replicas=len(self.workers)\
                    #                                   ,replicas_to_aggregate=len(self.workers),use_locking=True);
        
                    # self.final_optimizer_unsup = self.replicated_opt_unsup.minimize(self.final_cost_unsup, global_step=self.global_step);


                    # self.init_token_unsup = self.replicated_opt_unsup.get_init_tokens_op();##Returns the op to fill the sync_token_queue with the tokens.
    
                    # self.queue_runner_unsup = self.replicated_opt_unsup.get_chief_queue_runner(); #Returns the QueueRunner for the chief to execute.


                
        
                # self.init_token_unsupervised = self.replicated_opt_unsupervised.get_init_tokens_op();##Returns the op to fill the sync_token_queue with the tokens.
    
                # self.queue_runner_unsupervised = self.replicated_opt_unsupervised.get_chief_queue_runner(); #Returns the QueueRunner for the chief to execute.
                ...
                self.sv_obj = tf.train.Supervisor(is_chief=(self.FLAGS.task_index == 0),\
    ...
    def model_train(self,labeled_tr_inputs,labeled_tr_targets,unlabeled_tr_inputs,test_inputs,test_targets,num_epochs,batch_size,re_train,
        ...
        if self.FLAGS.job_name=='worker':
            # num_examples = len(labeled_tr_inputs)
            # num_batches_per_epoch = int(num_examples/batch_size)
            cost_plot_task0=[];
            cost_plot_task1=[];cost_plot_task2=[];

            test_cer_task0=[]; test_cer_task1=[]; test_cer_task2=[];
            ...
            with self.sv_obj.prepare_or_wait_for_session(self.server.target) as session:
                ...
                if self.FLAGS.task_index == 0:
                    ...
                    self.sv_obj.start_queue_runners(session, [self.queue_runner_sup]);
                    
                    session.run(self.init_token_sup);

                    # self.sv_obj.start_queue_runners(session, [self.queue_runner_unsup]);
                    
                    # session.run(self.init_token_unsup);
                ...
                for curr_epoch in range(num_epochs):
                    ...
                    for batch in range(num_batches_per_epoch):
                        ...
                        batch_cost, _ = session.run([self.final_cost_sup, self.final_optimizer_sup], feed)
                        
                        sup_train_cost += (batch_cost*batch_size);
                        print(batch_cost);
                        print('batch cost is {}'.format(batch_cost));
                        print('global_step: %s' % tf.train.global_step(session, self.global_step))
                        ...
                        if curr_epoch>1700:
                            ...
                            batch_cost, _ = session.run([self.final_cost_unsup, self.final_optimizer_unsup], feed)
                            unsup_train_cost+= (batch_cost*batch_size_unsup);
                        
                        # print('batch cost is {}'.format(batch_cost));
                        # print('Global Step is {}'.format(print(session.run(self.global_step))));
                    ...
                    # sup_train_cost 
                    # unsup_train_cost
                    if(curr_epoch%1==0 and sup_train_cost<350 and self.FLAGS.task_index==1 ):
                    ...
                    log_file = open('worker_bsp_log'+str(self.FLAGS.task_index)+'.txt',"a");
                ...
                #np.save('heir2',arr)
                    if self.FLAGS.task_index==0:
                        cost_plot_task0.append([sup_train_cost,unsup_train_cost]);
                        test_cer_task0.append(test_cer);

                    elif self.FLAGS.task_index==1:
                        cost_plot_task1.append([sup_train_cost,unsup_train_cost]);
                        test_cer_task1.append(test_cer);
                    else:
                        cost_plot_task2.append([sup_train_cost,unsup_train_cost]);
                        test_cer_task2.append(test_cer);
                ...
                ## for model saving
                    ...
                    if(curr_epoch==250):
                        self.saver.save(session, './model_cvt_same_batch_bsp_sup'+str(self.FLAGS.task_index)+'/model.ckpt');
                        print("Model saved in path: %s" % './model_cvt_same_batch_bsp_sup'+str(self.FLAGS.task_index)+'/model.ckpt');

            self.sv_obj.stop();
            # self.log_file.close();

            np.save(self.log_dir+'worker0.npy',cost_plot_task0);
            np.save(self.log_dir+'worker1.npy',cost_plot_task1);
            np.save(self.log_dir+'worker2.npy',cost_plot_task1);

            np.save(self.log_dir+'worker0_cer.npy',test_cer_task0);
            np.save(self.log_dir+'worker1_cer.npy',test_cer_task1);
            np.save(self.log_dir+'worker2_cer.npy',test_cer_task2);
        # print('Model trained')
