...
from cvt_same_batch_asp import BiGRU_CTC
import numpy as np
...
tf.app.flags.DEFINE_string("gpu_id","0",'GPU_ID');
...
def data_extraction(FLAGS):    ### num_features are 13*3=39 mfcc unnormalised
    

    # if flags.task_index==0:   ################### DATA SHARDING ################################################################################################

    
    train_file_l="/home/karanm/mlld/project/lstm-ctc_final/data_lists/shard_2_l_"+str(FLAGS.task_index+1); 
    train_file_u="/home/karanm/mlld/project/lstm-ctc_final/data_lists/shard_2_ul_"+str(FLAGS.task_index+1);
...
train_x_l,train_y_l,train_x_u,train_y_u,test_x,test_y=data_extraction(FLAGS);
...
initial_model_path='./model_cvt_same_batch_asp'+str(FLAGS.task_index)+'/model.ckpt'
initial_model_path_res='./model_cvt_same_batch_asp'+str(0)+'/model.ckpt'


ASR=BiGRU_CTC(FLAGS=FLAGS,num_features=num_features,initial_learning_rate=0.001,beam_width=20,top_paths=3); ## ASR object created
...
ASR.model_train(labeled_tr_inputs=x_l,labeled_tr_targets=y_l,unlabeled_tr_inputs=x_u,
                ...
                num_epochs=700,model_path_sav=initial_model_path,model_path_res=initial_model_path_res,re_train=True);
