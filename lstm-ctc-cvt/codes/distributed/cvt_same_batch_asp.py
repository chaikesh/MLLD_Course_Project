...
class BiGRU_CTC():
    ...
    def __init__(self,FLAGS,num_hidden = 200,num_layers = 2,num_features=40,initial_learning_rate = .001,
        ...
        self.parameter_servers=["10.1.1.254:2232"]; ##  node 0 is 254  node 1 is 253 node 2 is 252 and node 3 is 251 node 4 is 249 
        self.workers=["10.1.1.253:2272","10.1.1.252:2277"];   
...
# config.gpu_options.allow_growth=True;
    ...
    ###############################
                ...
                self.optimizer_supervised = tf.train.MomentumOptimizer(self.initial_learning_rate,0.9).minimize(self.final_cost_supervised,global_step=self.global_step);   
                ...
                self.optimizer_unsupervised = tf.train.MomentumOptimizer(self.initial_learning_rate/50,0.9).minimize(self.final_cost_unsupervised,global_step=self.global_step);
        ...
        if self.FLAGS.job_name=='worker':
            ...
            with self.sv_obj.prepare_or_wait_for_session(self.server.target) as session:
                ...
                for curr_epoch in range(num_epochs):
                    ...
                    if(curr_epoch%1==0 and sup_train_cost<350 and self.FLAGS.task_index==0):
                    ...
                    log_file = open('worker_asp_2_log'+str(self.FLAGS.task_index)+'.txt',"a");
                ...
                ## for model saving
                    ...
                    if(curr_epoch==250):
                        self.saver.save(session, './model_cvt_same_batch_asp_sup'+str(self.FLAGS.task_index)+'/model.ckpt');
                        print("Model saved in path: %s" % './model_cvt_same_batch_asp_sup'+str(self.FLAGS.task_index)+'/model.ckpt');
